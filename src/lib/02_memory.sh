...
prepare_memory_plan() {
    ...
    local total_mb cpu_count reserved_mb build_reserved_mb build_limit_mb
    local runtime_reserved_mb runtime_budget_mb database_mb world_mb auth_mb used_mb
    ...
    if [ "${AC_MEMORY_AUTO:-1}" = "0" ]; then
        ...
        set_default_if_empty DOCKER_BUILDKIT_MAX_PARALLELISM "1"
        ...
        export AC_MEMORY_TOTAL_MB DOCKER_BUILD_MEMORY_LIMIT DOCKER_BUILD_PARALLEL_JOBS
        export DOCKER_BUILDKIT_MAX_PARALLELISM AC_DATABASE_MEMORY_LIMIT
        export AC_WORLDSERVER_MEMORY_LIMIT AC_AUTHSERVER_MEMORY_LIMIT
        export AC_DATABASE_INNODB_BUFFER_POOL_SIZE AC_MEMORY_PLAN_READY
        return 0
    fi

    reserved_mb="${AC_MEMORY_RESERVED_MB:-1024}"
    runtime_reserved_mb="$reserved_mb"
    runtime_reserved_mb="$(memory_clamp "$runtime_reserved_mb" 768 "$((total_mb - 1024))")"

    build_reserved_mb="$reserved_mb"
    build_reserved_mb="$(memory_clamp "$build_reserved_mb" 1024 "$((total_mb - 1024))")"
    ...
    runtime_budget_mb="$(memory_clamp "$runtime_budget_mb" 1536 "$((total_mb - 512))")"
    ...
    used_mb=$((database_mb + world_mb + auth_mb))
    if [ "$used_mb" -gt "$runtime_budget_mb" ]; then
        world_mb=$((world_mb - (used_mb - runtime_budget_mb)))
        world_mb="$(memory_max "$world_mb" 1024)"
    fi

    max_jobs=$((build_limit_mb / 2048))
    max_jobs="$(memory_clamp "$max_jobs" 1 "$cpu_count")"
    if [ "$total_mb" -lt 4096 ]; then
        max_jobs="$(memory_min "$max_jobs" 1)"
    fi
    build_jobs="$max_jobs"
    ...
    AC_MEMORY_PLAN_READY=1
    export AC_MEMORY_TOTAL_MB AC_BUILD_RESERVED_MB AC_RUNTIME_RESERVED_MB AC_RUNTIME_MEMORY_BUDGET_MB
...
print_memory_plan() {
    prepare_memory_plan
    echo "内存规划: 有效内存=${AC_MEMORY_TOTAL_MB}MB 构建=${DOCKER_BUILD_MEMORY_LIMIT} 构建并行=${DOCKER_BUILD_PARALLEL_JOBS} 运行预算=${AC_RUNTIME_MEMORY_BUDGET_MB:-自动}MB"
    echo "运行限制: database=${AC_DATABASE_MEMORY_LIMIT} worldserver=${AC_WORLDSERVER_MEMORY_LIMIT} authserver=${AC_AUTHSERVER_MEMORY_LIMIT} innodb_buffer_pool=${AC_DATABASE_INNODB_BUFFER_POOL_SIZE}"
}
